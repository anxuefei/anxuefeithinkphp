...
class Cart extends Controller {
	...
	private function save( $goods ) {
		Session::set( $this->cartName . '.goods', $goods );
		Session::set( $this->cartName . '.total_rows', $this->getTotalNums() );
		Session::set( $this->cartName . '.total', $this->getTotalPrice() );
	}
		...
		foreach ( $data as $dataOne ) {
			foreach ( $goods as $k => $v ) {
				if ( $k == $dataOne['sid'] ) {
					...
					$goods[ $k ]['total'] = $dataOne['num'] * $v['price'];
